...
func init() {
	...
	regexDataTypePatterns["(unsigned\\s){0,1}(double.*|float.*|dec.*|real)"] = "float64"
	regexDataTypePatterns["varchar.*|date.*|time.*|year.*|char.*|.*text.*|enum.*|set.*|.*blob.*|.*binary.*|point"] = "string"
}
